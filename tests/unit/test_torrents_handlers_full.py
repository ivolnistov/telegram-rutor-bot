...
async def test_torrent_info_v2(mocker, tg_update, tg_context, mock_db_user):
    ...
    mocker.patch(
        'telegram_rutor_bot.handlers.torrents.get_torrent_by_id', AsyncMock(return_value=MagicMock(link='l', name='T'))
    )
    mocker.patch(
        'telegram_rutor_bot.handlers.torrents.get_torrent_info', AsyncMock(return_value=('Msg', None, [], None, {}))
    )
