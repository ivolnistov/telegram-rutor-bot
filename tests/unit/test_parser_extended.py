...
async def test_get_torrent_info_error(mocker):
    ...
    mocker.patch("httpx.AsyncClient", return_value=MagicMock(__aenter__=AsyncMock(return_value=mock_client), __aexit__=AsyncMock()))
    pass
...
async def test_download_torrent_success(mocker):
    from telegram_rutor_bot.db.models import Torrent, Film
    ...
    mock_client = AsyncMock()
    mock_client.get.return_value = mock_response
    mocker.patch("telegram_rutor_bot.rutor.parser._get_client", return_value=mocker.MagicMock(__aenter__=AsyncMock(return_value=mock_client), __aexit__=AsyncMock()))
