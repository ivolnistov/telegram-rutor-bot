...
from telegram_rutor_bot.db.models import User, Film, Torrent
...
def test_discovery_endpoints_extended(mocker, mock_session):
    ...
    mocker.patch("telegram_rutor_bot.api.routes.discovery.tmdb.get_account_states", AsyncMock(return_value={"rated": 5}))
    
    # Mock library status logic
    ...
    assert client.post("/api/discovery/rate", json={"media_type": "movie", "media_id": 1, "value": 8.0}).status_code == 200
    ...
    # Test library
    mock_film = MagicMock(spec=Film, id=1, tmdb_id=100, name="F", original_title="O", poster="P", rating=8.0, year=2024, tmdb_media_type="movie", country="USA", kp_rating=8.5)
    ...
    # Test personal
    mocker.patch("telegram_rutor_bot.api.routes.discovery.tmdb.get_personal_recommendations", AsyncMock(return_value=[]))
    ...
    # Test search_on_rutor
    mocker.patch("telegram_rutor_bot.api.routes.discovery.search_film_on_rutor.kiq", AsyncMock())
    mock_session.execute.return_value.scalar_one_or_none.side_effect = [None, mock_film]
