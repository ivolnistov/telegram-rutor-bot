...
async def test_callback_torrents_variants(mocker):
    ...
    # Mock session
    mocker.patch("telegram_rutor_bot.handlers.torrents.get_async_session", return_value=MagicMock(__aenter__=AsyncMock(return_value=AsyncMock()), __aexit__=AsyncMock()))
    ...
    mocker.patch("telegram_rutor_bot.handlers.torrents.get_torrent_info", AsyncMock(return_value=("", None, [], None, {})))
    await callback_query_handler(update, context)
