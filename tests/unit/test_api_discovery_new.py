...
async def test_get_trending_api(mocker):
    mocker.patch(
        'telegram_rutor_bot.api.routes.discovery.tmdb.get_trending', AsyncMock(return_value=[{'id': 1, 'title': 'T'}])
    )
    mock_db = AsyncMock()
...
async def test_search_discovery_api(mocker):
    mocker.patch(
        'telegram_rutor_bot.api.routes.discovery.tmdb.search_multi',
        AsyncMock(return_value=[{'id': 1, 'title': 'T', 'media_type': 'movie'}]),
    )
    mock_db = AsyncMock()
    mock_result = MagicMock()
    mock_result.all.return_value = []
