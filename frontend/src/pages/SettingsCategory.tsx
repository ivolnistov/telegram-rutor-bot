...
const CategoryModal = ({
  ...
  const createMut = useMutation({
    ...
    onError: (e: AxiosError<{ detail: string }>) => {
      alert(e.response?.data.detail || t('categories.form.error_create'))
    },
  })
  ...
  const updateMut = useMutation({
    ...
    onError: (e: AxiosError<{ detail: string }>) => {
      alert(e.response?.data.detail || t('categories.form.error_update'))
    },
  })

  const handleSubmit = async () => {
    if (!name) {
      alert(t('categories.form.name_required'))
      return
    }
    setIsLoading(true)
    try {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Input
            ...
            className="text-zinc-200"
            placeholder={t('categories.form.folder_placeholder')}
          />
        </div>
...
const SettingsCategory = () => {
  ...
  const deleteMut = useMutation({
    ...
    onError: (e: AxiosError<{ detail: string }>) => {
      alert(e.response?.data.detail || t('categories.form.error_delete'))
    },
  })
          ...
          >
            ...
            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <Button
                ...
                onClick={() => {
                  if (confirm(t('categories.form.delete_confirm')))
