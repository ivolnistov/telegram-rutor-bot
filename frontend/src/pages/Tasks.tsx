                    ...
                    <Card key={task.id} className="flex items-center justify-between py-3 px-4">
                        <div className="flex items-center gap-4">
                            <div className="flex flex-col">
                                <span className={`text-sm font-medium ${task.status === 'success' ? 'text-zinc-200' :
                                    task.status === 'failed' ? 'text-red-400' :
                                        task.status === 'pending' ? 'text-zinc-500' : 'text-blue-400'
                                    }`}>
                        ...
                        <div className="flex flex-col items-end gap-1 min-w-[200px]">
                            ...
                            <div className="flex items-center gap-2">
                                <span className={`size-2.5  rounded-full ${task.status === 'pending' ? 'bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.5)]' :
                                    task.status === 'running' ? 'bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)] animate-pulse' :
                                        task.status === 'success' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' :
                                            'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]'
                                    }`} />
                                <span className={`text-xs font-medium ${task.status === 'pending' ? 'text-yellow-500' :
                                    task.status === 'running' ? 'text-blue-500' :
                                        task.status === 'success' ? 'text-emerald-500' :
                                            'text-red-500'
                                    }`}>
                                    {task.status === 'pending' ? t('tasks.pending') :
                                        task.status === 'running' ? t('tasks.running') :
                                            task.status === 'success' ? t('tasks.success') : t('tasks.failed')}
                                </span>
                            </div>
                            ...
                            {(task.result || task.status === 'running') && (
                                <div className="text-xs text-zinc-400 font-mono mt-1 text-right">
                                    {task.status === 'running' ? (
                                        ...
                                        <div dangerouslySetInnerHTML={{ __html: task.result || '' }} />
                                    )}
                                </div>
                            )}

                            <div className="text-[10px] text-zinc-600 font-mono mt-1">
                                {t('tasks.duration')}: {getDuration(task.start_time, task.end_time)} • ID: {task.id}
                            </div>
                        </div>
                    </Card>
                ))}
