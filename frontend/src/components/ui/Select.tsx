...
export const Select = ({ value, onChange, options, placeholder = 'Select...', className, startIcon, disabled }: SelectProps) => {
        ...
        <div className={twMerge("relative w-full", className)} ref={containerRef}>
            <button
                ...
                className={clsx(
                    ...
                    !disabled && "hover:border-zinc-700 hover:bg-zinc-900/80 focus:outline-none focus:ring-1 focus:ring-violet-500 focus:border-violet-500",
                    isOpen && "border-violet-500 ring-1 ring-violet-500",
                    disabled && "opacity-50 cursor-not-allowed"
                )}
            >
                <span className={clsx("flex items-center gap-2 truncate", !selectedOption && "text-zinc-500")}>
                    {selectedOption?.icon || startIcon}
                    {selectedOption ? selectedOption.label : placeholder}
                </span>
                <ChevronDown className={clsx("size-4  text-zinc-500 transition-transform", isOpen && "rotate-180")} />
            </button>
