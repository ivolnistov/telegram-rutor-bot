...
export const CronScheduler = ({
  ...
  const [preferredMode, setPreferredMode] = useState<'scheduled' | 'raw'>(
    'scheduled',
  )
  ...
  const parseCron = (cron: string) => {
    try {
      ...
      else if (dom.startsWith('*/') && mon === '*' && dow === '*')
        f = `${dom.substring(2)}_days`
      else if (dom === '*' && mon === '*' && dow === '1')
        f = '1_week' // Weekly on Monday
      else if (dom === '1' && mon === '*' && dow === '*') f = '1_month'
      else if (dom === '1' && mon.startsWith('*/') && dow === '*')
        f = `${mon.substring(2)}_months`
  ...
  const generateCron = (freq: string, t: string) => {
    ...
    switch (freq) {
      case '1_day':
        return `${String(mm)} ${String(hh)} * * *`
      case '2_days':
        return `${String(mm)} ${String(hh)} */2 * *`
      case '3_days':
        return `${String(mm)} ${String(hh)} */3 * *`
      case '4_days':
        return `${String(mm)} ${String(hh)} */4 * *`
      case '5_days':
        return `${String(mm)} ${String(hh)} */5 * *`
      case '6_days':
        return `${String(mm)} ${String(hh)} */6 * *`
      case '7_days':
        return `${String(mm)} ${String(hh)} */7 * *`
      case '1_week':
        return `${String(mm)} ${String(hh)} * * 1`
      case '2_weeks':
        return `${String(mm)} ${String(hh)} */14 * *`
      case '3_weeks':
        return `${String(mm)} ${String(hh)} */21 * *`
      case '4_weeks':
        return `${String(mm)} ${String(hh)} */28 * *`
      case '1_month':
        return `${String(mm)} ${String(hh)} 1 * *`
      case '2_months':
        return `${String(mm)} ${String(hh)} 1 */2 *`
      case '3_months':
        return `${String(mm)} ${String(hh)} 1 */3 *`
      case '6_months':
        return `${String(mm)} ${String(hh)} 1 */6 *`
      case '1_year':
        return `${String(mm)} ${String(hh)} 1 1 *`
      default:
        return value
    }
  }
        ...
        <div className="animate-in fade-in slide-in-from-top-2 duration-200 grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              value={time}
              onChange={(e) => {
                handleUIChange(frequency, e.target.value)
              }}
